...
<html>
    ...
    <body id="noScrollBackground">
        ...
        <br>
        <%--Adding icon below:--%>
        <img id="timeImage" src="time.png" />
        <%--Creating header below:--%>
        <p id="announcementText">TIMER</p>
        <br>
        <p id="timeText">COUNTDOWN</p>
        <br>
        <!--<form action="countdown" method="POST">-->
        <input id="timeEntry" type="text" onfocus="this.value = ''" value="ENTER TIME HERE"/>
        <br>
        <br>
        <br>
        <%--Creating button to insert data into database below:--%>
        <div id="startTimeButton">
            <input id="mockButtonTest" type="button" onclick="timer(document.getElementById('timeEntry').value)" value="START"/>
        </div>
        <!--</form>-->
        <%--Creating button to insert data into database below:--%>
        <form id="startBackButton" action="backToMockProcedures" method="POST">
